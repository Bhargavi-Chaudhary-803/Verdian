          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"flex", background:C.surface, borderRadius:10, padding:4, border:`1px solid ${C.border}` }}>
              <button onClick={() => setRole("user")}
                style={{ flex:1, padding:"9px 0", borderRadius:8, border:"none", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:600, fontSize:13, transition:"all .2s",
                  background:role==="user"?C.accent:"transparent", color:role==="user"?"#0a0f0d":C.muted, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                <Leaf size={14}/> Citizen
              </button>
              <button onClick={() => setRole("admin")}
                style={{ flex:1, padding:"9px 0", borderRadius:8, border:"none", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontWeight:600, fontSize:13, transition:"all .2s",
                  background:role==="admin"?"#ef4444":"transparent", color:role==="admin"?"#fff":C.muted, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                <Shield size={14}/> Admin
              </button>
            </div>
            {role==="admin" && isLogin && (
              <div style={{ marginTop:8, padding:"8px 12px", borderRadius:8, background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)", fontSize:12, color:"#ef4444" }}>
                Signing in as Admin — ensure your account has admin role in Supabase.
              </div>
            )}
          </div>
...
function UserDashboard({ user }) {
  ...
  const weekBars = (() => {
    const d = new Array(7).fill(0); const now = new Date();
    logs.forEach(l => { const diff = Math.floor((now-new Date(l.created_at))/86400000); if(diff<7) d[6-diff]++; });
    return d;
  })();
    ...
    <div className="fade-in" style={{ padding:28, display:"flex", flexDirection:"column", gap:22 }}>
      ...
      <div style={{ position:"relative", borderRadius:20, overflow:"hidden", padding:"28px 32px", background:"linear-gradient(135deg,#091409 0%,#0a1a0e 60%,#061009 100%)", border:"1px solid rgba(34,197,94,0.12)" }}>
        <svg style={{ position:"absolute", right:0, top:0, height:"100%", width:"45%", opacity:1 }} viewBox="0 0 380 160" preserveAspectRatio="xMaxYMid slice">
          <defs><radialGradient id="bg1" cx="60%" cy="50%"><stop offset="0%" stopColor="#22c55e" stopOpacity="0.18"/><stop offset="100%" stopColor="#22c55e" stopOpacity="0"/></radialGradient></defs>
          <ellipse cx="280" cy="80" rx="160" ry="100" fill="url(#bg1)"/>
          ...
          {[0,45,90,135,180,225,270,315].map((deg,i)=>{ const r=deg*Math.PI/180,x=300+70*Math.cos(r),y=80+70*Math.sin(r); return <circle key={i} cx={x} cy={y} r="2.5" fill="#22c55e" opacity={i%2===0?0.5:0.25}/>; })}
        </svg>
        <div style={{ position:"relative", zIndex:1 }}>
          <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6 }}>
            <div style={{ width:6, height:6, borderRadius:"50%", background:"#22c55e", boxShadow:"0 0 6px #22c55e" }} />
            <span style={{ fontSize:12, color:"#22c55e", fontWeight:700, letterSpacing:"0.08em" }}>CITIZEN DASHBOARD</span>
          </div>
          <div className="syne" style={{ fontWeight:800, fontSize:26, color:C.text, marginBottom:6 }}>Hi, {profile.name?.split(" ")[0]}!</div>
          <div style={{ fontSize:13, color:C.muted, marginBottom:20 }}>
            You've saved <span style={{ color:C.accent, fontWeight:700 }}>{profile.co2_saved||0} kg</span> CO₂ · Rank <span style={{ color:C.blue, fontWeight:700 }}>#{rank||"—"}</span> in Delhi
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {[
              [Award,   "Eco Hero",                   "rgba(34,197,94,0.1)","rgba(34,197,94,0.2)",  C.accent],
              [Activity,`${profile.streak||0}d streak`,"rgba(245,158,11,0.1)","rgba(245,158,11,0.2)",C.warn],
              [Star,    `${profile.green_points||0} pts`,"rgba(59,130,246,0.1)","rgba(59,130,246,0.2)",C.blue],
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14 }}>
        {[
          ...
          { label:"Green Points", value:loading?0:profile.green_points||0,   suffix:"",    icon:Star,      color:C.accent, max:500 },
          { label:"CO₂ Saved",    value:loading?0:profile.co2_saved||0,      suffix:" kg", icon:Leaf,      color:"#4ade80",max:50 },
          { label:"Day Streak",   value:loading?0:profile.streak||0,         suffix:"d",   icon:Activity,  color:C.warn,   max:30 },
        ].map((s,i)=>{
          const pct=Math.min((s.value/s.max)*100,100), r=24, circ=2*Math.PI*r, filled=tick?(pct/100)*circ:0;
          return (
            <div key={i} style={{ position:"relative", padding:"18px 20px", borderRadius:16, background:C.card, border:`1px solid ${C.border}`, overflow:"hidden", transition:"transform .2s,box-shadow .2s" }}
              ...
              <div style={{ position:"relative", display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                <div>
                  ...
                  <div className="syne" style={{ fontSize:28, fontWeight:800, color:C.text, lineHeight:1 }}>{loading?"—":s.value}{s.suffix}</div>
                </div>
              ...
              <div style={{ marginTop:6, fontSize:12, color:C.muted }}>{Math.round(pct)}% of {s.suffix?s.max+s.suffix:`${s.max} target`}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18 }}>
        <div style={{ padding:22, borderRadius:16, background:C.card, border:`1px solid ${C.border}` }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            <div className="syne" style={{ fontWeight:700, fontSize:14, color:C.text }}>Recent Activity</div>
            <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:C.accent, fontWeight:700 }}>
              <div style={{ width:5, height:5, borderRadius:"50%", background:C.accent, animation:"pulse 2s infinite" }}/> LIVE
            </div>
          </div>
            ...
            logs.length===0 ? (
              <div style={{ textAlign:"center", padding:"28px 0" }}>
                <div style={{ width:48, height:48, borderRadius:12, background:"rgba(34,197,94,0.06)", border:"1px solid rgba(34,197,94,0.1)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 12px" }}>
                  <ScanLine size={22} color={C.dim}/>
                </div>
                <div style={{ color:C.muted, fontSize:13 }}>No waste logged yet. Start with the AI Scanner!</div>
              </div>
            ) : (
                ...
                {logs.map((log,i)=>(
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:10, background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.04)", transition:"all .2s" }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:13, color:C.text, fontWeight:500, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{log.item_name}</div>
                      <div style={{ fontSize:12, color:C.muted }}>{log.category} · {new Date(log.created_at).toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</div>
                    </div>
                    <div style={{ padding:"2px 8px", borderRadius:100, background:"rgba(34,197,94,0.1)", fontSize:12, color:C.accent, fontWeight:700, flexShrink:0 }}>+{log.points_earned}pts</div>
                  </div>
                ))}
              </div>
            )
        ...
        <div style={{ padding:22, borderRadius:16, background:C.card, border:`1px solid ${C.border}` }}>
          <div className="syne" style={{ fontWeight:700, fontSize:14, color:C.text, marginBottom:4 }}>This Week</div>
          <div style={{ fontSize:12, color:C.muted, marginBottom:20 }}>Items logged per day</div>
          <div style={{ display:"flex", alignItems:"flex-end", gap:8, height:100 }}>
            {weekBars.map((v,i)=>{ const days=["M","T","W","T","F","S","S"]; const isMax=v===maxBar&&v>0;
              return (<div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:5 }}>
                ...
                <div style={{ fontSize:12, color:isMax?C.accent:C.muted, fontWeight:isMax?700:400 }}>{days[i]}</div>
              </div>);
            })}
          </div>
          <div style={{ marginTop:20, paddingTop:16, borderTop:`1px solid rgba(34,197,94,0.08)` }}>
            <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:C.muted, marginBottom:10, fontWeight:600 }}><Leaf size={12} color={C.muted}/> Quick Guide</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
              {wasteCategories.map((wc,i)=>(
                <div key={i} style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 8px", borderRadius:8, background:`${wc.color}08`, border:`1px solid ${wc.color}15` }}>
                  <wc.icon size={12} color={wc.color}/><span style={{ fontSize:12, color:wc.color, fontWeight:600 }}>{wc.label}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                <div style={{ padding:24, borderRadius:16, background:`${meta.color}12`, border:`1px solid ${meta.color}30`, marginBottom:16 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12 }}>
                    ...
                    <div style={{ textAlign:"right" }}>
                      <div className="syne" style={{ fontSize:28, fontWeight:800, color:meta.color }}>{selectedItem.confidence}%</div>
...
function MapView() {
  ...
  const loadMapData = async () => {
    ...
    setHotspots(hs||[]); setWasteLogs(logs||[]); setLoading(false);
  };

  useEffect(() => {
    loadMapData();
    // Realtime: refresh log pins when new geotagged waste is added
    const sub = supabase.channel("map-logs")
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"waste_logs"},()=>loadMapData())
      .subscribe();
  ...
  const renderHotspotMarkers = (map,spots,fil) => {
    ...
    hotspotMarkersRef.current.forEach(m=>map.removeLayer(m)); hotspotMarkersRef.current=[];
    spots.filter(h=>fil==="all"||h.level===fil).forEach(h=>{
      const col=levelColor[h.level]||"#22c55e";
      const icon=L.divIcon({ className:"",
        html:`<div style="width:34px;height:34px;border-radius:50%;background:${col}22;border:2.5px solid ${col};display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px ${col}44;"><div style="width:12px;height:12px;border-radius:50%;background:${col};box-shadow:0 0 6px ${col};"></div></div>`,
  ...
  const renderLogMarkers = (map,logs) => {
    const L=window.L; if(!L||!map) return;
    logMarkersRef.current.forEach(m=>map.removeLayer(m)); logMarkersRef.current=[];
    logs.forEach(log=>{
      if(!log.lat||!log.lng) return;
      const col=catColor[log.category]||"#6b8c78";
      const wc=wasteCategories.find(w=>w.id===log.category);
      // SVG icon using a lucide-style path for each category
      const iconHtml=`<div style="width:30px;height:30px;border-radius:8px;background:white;border:2px solid ${col};display:flex;align-items:center;justify-content:center;box-shadow:0 2px 6px rgba(0,0,0,0.15);cursor:pointer;">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="${col}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ${log.category==="recyclable"?'<polyline points="1 4 1 10 7 10"/><polyline points="23 20 23 14 17 14"/><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15"/>':
            log.category==="organic"?'<path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6C9.5 14.52 12 13 13 12"/>':
            log.category==="hazardous"?'<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>':
            '<polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/>'}
        </svg>
      </div>`;
      const icon=L.divIcon({ className:"", html:iconHtml, iconSize:[30,30], iconAnchor:[15,15], popupAnchor:[0,-18] });
  ...
  useEffect(() => { if(mapObjRef.current&&hotspots.length>0) renderHotspotMarkers(mapObjRef.current,hotspots,filter); }, [filter,hotspots]);
  useEffect(() => {
    if(!mapObjRef.current) return;
    logMarkersRef.current.forEach(m=>{ if(showLogs) mapObjRef.current.addLayer(m); else mapObjRef.current.removeLayer(m); });
  }, [showLogs]);
  // Re-render log markers when wasteLogs updates (realtime)
  useEffect(() => { if(mapObjRef.current&&!loading) renderLogMarkers(mapObjRef.current,wasteLogs); }, [wasteLogs]);
...
function ScannerLocation({ scannerLocation, setScannerLocation }) {
  ...
  if (scannerLocation) return (
    <div style={{ marginTop:12, display:"flex", alignItems:"center", gap:8, padding:"8px 12px", borderRadius:8, background:"rgba(34,197,94,0.06)", border:`1px solid ${C.dim}`, fontSize:12 }}>
      ...
      <button onClick={()=>setScannerLocation(null)} style={{ background:"none", border:"none", cursor:"pointer", color:C.muted, marginLeft:"auto", padding:2 }}><X size={12}/></button>
    </div>
  );
          ...
          {tab==="gps" ? (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              <div style={{ display:"flex", gap:6 }}>
                ...
                <button onClick={handleAddressSearch} disabled={searching}
                  style={{ padding:"7px 12px", borderRadius:7, background:C.accent, border:"none", color:"#0a0f0d", cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>
      ...
      {loading ? <div style={{ textAlign:"center", padding:60 }}><Spinner size={32}/></div> : (<>
        ...
        {isAdmin&&leaderboard.length>0&&(
          <div className="card" style={{ padding:24 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"40px 1fr 120px 100px" }}>
        ...
        {isAdmin&&(
          <div className="card" style={{ padding:24 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {scheduleItems.map((s,i)=>{ const sc=s.status==="completed"?C.accent:s.status==="in-progress"?C.warn:C.muted; return (
...
function AdminDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setHotspots(hs||[]); setUserCount(uc||0); setLogCount(lc||0); setLoading(false);
    };
    load();
      ...
      {loading ? <div style={{ textAlign:"center", padding:40 }}><Spinner size={32}/></div> : (<>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          ...
          <div className="card" style={{ padding:24 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {scheduleItems.slice(0,4).map((s,i)=>{ const sc=s.status==="completed"?C.accent:s.status==="in-progress"?C.warn:C.muted; return (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:10, background:C.surface, border:`1px solid ${C.border}` }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:C.muted }}>{s.time} · {s.type}</div>
                  </div>
                  <div style={{ padding:"2px 8px", borderRadius:100, background:`${sc}18`, fontSize:12, color:sc, fontWeight:600, textTransform:"capitalize" }}>{s.status}</div>
                </div>
              ); })}
            </div>
          </div>
  ...
  return (
    <div className="fade-in" style={{ padding:28, display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {hotspots.map((h,i)=>{ const col=levelColor[h.level]; return (
